...
def register():   #get 요청 단순히 페이지 표시 post요청 회원가입-등록을 눌렀을때 정보 가져오는것
    ...
    if form.validate_on_submit(): # POST검사의 유효성검사가 정상적으로 되었는지 확인할 수 있다. 입력 안한것들이 있는지 확인됨.
        ...
        fcuser.useremail = form.data.get('useremail')
        fcuser.userlocation = form.data.get('userlocation')
...
def hello():
    ...
    return render_template('index.html',userid=userid)    # 이번 포스팅에는 필요없음(지난포스팅꺼)
...
#수정사항에 대한 track을 하지 않는다. True로 한다면 warning 메시지유발
    ...
    app.run(host='192.168.171.197', port=5025, debug=True) 
